...
// 姓（セイ）の場合
if(empty($sei)){
    $errorssei1 = '姓（セイ）は必須項目です。';
  } elseif (preg_match("/^[ァ-ヶー]+$/u", $sei)) {
    $errorssei2 =  '全角カナで入力してください。';
  }

// 名（メイ）の場合
if(empty($mei)){
    $errorsmei1 =  '名（セイ）は必須項目です。';
  } elseif (preg_match("/^[ァ-ヶー]+$/u", $mei)) {
    $errorsmei2 =  '全角カナで入力してください。';
  }

// メールアドレスの場合
if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
  $errorsemail =  '正しいEメールアドレスを指定してください。';
}
?>
